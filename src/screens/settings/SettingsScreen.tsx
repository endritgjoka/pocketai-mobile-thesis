    ...
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Section title="Context prioritization">
          ...
          {settings.contextPrioritizationEnabled ? <Text style={styles.inlineHelp}>pocketai is the proposed algorithm; the other three are baselines for comparison.</Text> : null}
        </Section>
...
function StringPickerRow({ label, value, options, onPick }: { label: string; value: string; options: string[]; onPick: (value: string) => void }) { return <View style={styles.pickerRow}><View style={styles.pickerHeader}><Text style={styles.rowLabel}>{label}</Text></View><View style={styles.segmented}>{options.map((option) => <Pressable key={option} onPress={() => onPick(option)} style={[styles.segment, option === value && styles.segmentActive]}><Text numberOfLines={1} style={[styles.segmentText, option === value && styles.segmentTextActive]}>{option}</Text></Pressable>)}</View></View>; }
